...
//２．データ取得SQL作成
$stmt = $pdo->prepare("SELECT * FROM twitter_account JOIN follow_list on twitter_account.id = follow_list.followed_by WHERE username=:username ORDER BY follow_id DESC");
...
if($status==false) {
  ...
  while( $result = $stmt->fetch(PDO::FETCH_ASSOC)){ 
    array_push($following_accounts,$result["is_followed"]);
  }
}
